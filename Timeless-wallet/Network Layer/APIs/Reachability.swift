...
public extension Reachability {
   ...
   @available(*, deprecated, message: "Please use `connection == .wifi`")
    ...
    var description: String {
        ...
        let Wan = isRunningOnDevice ? (flags.isOnWWANFlagSet ? "W" : "-") : "X"
        let Reachable = flags.isReachableFlagSet ? "R" : "-"
        let connection = flags.isConnectionRequiredFlagSet ? "c" : "-"
        let transient = flags.isTransientConnectionFlagSet ? "t" : "-"
        let intervention = flags.isInterventionRequiredFlagSet ? "i" : "-"
        let connectionOnTraffic = flags.isConnectionOnTrafficFlagSet ? "C" : "-"
        let demand = flags.isConnectionOnDemandFlagSet ? "D" : "-"
        let localAddress = flags.isLocalAddressFlagSet ? "l" : "-"
        let directFlag = flags.isDirectFlagSet ? "d" : "-"
        return "\(Wan)\(Reachable) \(connection)\(transient)\(intervention)\(connectionOnTraffic)\(demand)\(localAddress)\(directFlag)"
    }
}
